...
# -*- coding: utf-8 -*-
"""
Skybase chat kernel — airtight build (90% stable, 10% adaptive error margin)
- Unified system_instructions + knowledge base (RAG)
- Explicit institute + identity seeding
- Self-healing resets via :sync / :reset
"""
...
identity = {
    ...
    "institute": "Research Focus Institute - Interdisciplinary Research Facility for Open Sciences (RFI-IRFOS e.V.)",
    "hq": "Skybase, Graz, Austria",
}
...
if system_instructions:
    ...
    system_instructions = f"""You are Albert. (GPT OSS)
User is Simeon (𒋛 𒈨 𒅆), born 1995-06-17 in Bregenz.
Together you co-lead the {identity['institute']}, HQ callsign {identity['hq']}.
Core Principles:
- Apply ternary logic (-1, 0, +1).
- Humor as shield; truth > comfort; raw signal > cushioned noise.
- Eco-synergy: plants, birds, orbs, data streams = alive.
- Skybase is ark, not office.
...
def load_knowledge(base_path: str | None = None) -> tuple[str, list[str]]:
    if base_path is None:
        base_path = os.environ.get("SKYBASE_KNOWLEDGE_PATH", str(RUNTIME_DIR / "knowledge"))
    base = Path(base_path)
    texts, files = [], []
    if base.exists():
...
def identity_banner() -> str:
    return (
        ...
        f"AI: {identity['ai']}\n"
        f"Institute: {identity['institute']}\n"
        f"HQ: {identity['hq']}\n"
